...
- **Debug logging convention**: The client SDK uses the `"smello"` Python logger hierarchy for debug output (`_debug.py`, patches, transport). All debug messages follow the pattern: `<past-tense-verb> <object> [(<detail>)]` — lowercase, no trailing period. Examples: `patched requests.Session.send`, `skipped GET example.com (ignored host)`, `captured POST api.stripe.com via httpx (201)`, `sent /api/capture/http (201)`. Config provenance is a single line: `resolved config: server_url=http://... (SMELLO_URL), capture_all=True (default), ...`. The `"smello"` logger has `propagate = False` and a `NullHandler` by default; `debug=True` adds a `StreamHandler(stderr)`. Child loggers (`smello.patches.*`, `smello.transport`) propagate to it. New debug log statements in patches/transport should follow this convention.
